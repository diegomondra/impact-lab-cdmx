...
def _(mo):
    mo.md(
        ...
        .obra-hero:after {
            ...
            position: absolute;
            inset: 0;
            background: linear-gradient(45deg, rgba(255, 255, 255, 0.09) 0%, transparent 40%), linear-gradient(135deg, rgba(255, 255, 255, 0.07) 0%, transparent 48%);
            pointer-events: none;
        }
        .obra-hero * {
            position: relative;
        ...
        <div style="font-family:'IBM Plex Sans', sans-serif; font-size: 15px; margin-top: 12px; opacity: 0.96; max-width: 840px; line-height: 1.65; color:#f1dce8;">
            Cada punto representa un proyecto financiado con recursos federales. Navega por tu colonia,
...
def _(DATA_DIR, pl):
    ...
    raw = raw.with_columns(
        [
            ...
            pl.col("monto_pagado").cast(pl.Float64, strict=False).fill_null(0),
            pl.col("avance_fisico").cast(pl.Float64, strict=False).fill_null(0),
            pl.col("poblacion_beneficiada").cast(pl.Float64, strict=False).fill_null(0),
        ]
    ).filter(
...
def _(df, fmt_int, fmt_mxn, mo, pl):
    ...
    _aprobado = df["monto_aprobado"].sum()
    _ejecucion = f"{_monto / _aprobado * 100:.1f}%" if _aprobado else "—"
    _terminados = df.filter(pl.col("avance_fisico") >= 95).height
